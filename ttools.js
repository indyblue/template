/* global Element, Text, Comment, DocumentFragment, Attr, Range */
'use strict';
...
(function() {
  ...
  _.rxAtt = function(rx) {
    return new RegExp('^(?:data-|x-)?' + rx.source, rx.flags);
  };
  ...
  _.isAttached = function(el) {
    ...
    return !!el.closest('body');
  };
  _.isSingleTextChild = function(obj) {
    return _.isElement(obj) && obj.childNodes.length === 1
      && _.isText(obj.firstChild);
  };
  ...
  _.getRange = function(first, last) {
    if (!first) return;
    if (_.isDocFrag(first)) {
      last = first.lastChild; first = first.firstChild;
    }
  ...
  _.elRemove = function(el) {
    if (_.isRange(el) && el.resetBounds) {
      return el.resetBounds().extractContents();
  ...
  _.elCheck = function(el) {
    ...
    if (!_.isElement(el) && !_.isDocFrag(el)
      && !_.isText(el) && !_.isComment(el)) { return void 0; }
  ...
  var thenArr = []; var lastArr = [];
  var thenState = false; var thenStart = null;
